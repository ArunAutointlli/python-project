...
# Database
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:123789@localhost/third'
...
def create_user():
    ...
    if isinstance(data, list):
        ...
        for user_data in data:
            new_user = colour(username=user_data['username'], email=user_data['email'], address=user_data['address'])
            db.session.add(new_user)

        db.session.commit()
        return jsonify({'MESSAGE': 'view CREATED SUCCESSFULLY'})
    else:
        # Handle the case when a single user is sent
        new_user = colour(username=data['username'], email=data['email'], address=data['address'])
        db.session.add(new_user)
        db.session.commit()
...
@app.route('/view/<int:user_id>', methods=['PUT'])
...
@app.route('/view/<int:user_id>', methods=['DELETE'])
