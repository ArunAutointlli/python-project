...
# Database
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:arun12@localhost/Queries'
...
def create_user():
    ...
    if isinstance(data, list):
        ...
        for user_data in data:
            new_user = Sev(username=user_data['username'], email=user_data['email'])
        ...
        new_user = Sev(username=data['username'], email=data['email'])
        db.session.add(new_user)
        db.session.commit()
